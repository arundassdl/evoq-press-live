...
class DatabaseServer(BaseServer):
	...
	def _setup_server(self):
		...
		try:
			...
			# Reconnect database after long-running playbook
			try:
			...
			# Update status based on play result
			...
			# Ensure database reconnection on error
			try:
				frappe.db.sql("SELECT 1")
			except Exception:
				try:
			...
			# Reload and save to persist error status
			try:
				self.reload()
				self.flags.ignore_version = True
				self.save(ignore_permissions=True)
				...
				frappe.log_error(
					title="Failed to save server after setup error",
					message=f"Server: {self.name}, Error: {str(save_error)}"
				)
